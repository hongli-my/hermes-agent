...
def validate_workflow(wf: Dict[str, Any]) -> List[str]:
    ...
    for step_id, node in nodes.items():
        ...
        if node.get("type") == "iteration":
            ...
            # 并发迭代禁止 sub_step 配置 workdir，只能在迭代层统一配置
            max_concurrent = node.get("max_concurrent", 1)
            if max_concurrent and max_concurrent > 1:
                for sub in node.get("sub_steps", []):
                    if sub.get("workdir"):
                        errors.append(
                            f"Iteration node '{step_id}' 配置了并发(max_concurrent={max_concurrent})，"
                            f"sub_step '{sub.get('id', '?')}' 不允许配置 workdir，"
                            f"请在迭代层统一配置 workdir"
                        )
