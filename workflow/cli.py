...
# 串行化 live-progress 的 print，避免并发 iteration 下 stdout 被截断/丢行
...
def _live_progress_callback(node_id: str, node_type: str, status: str,
    ...
    缩进层级（start/end 对齐）：
      顶层节点:       "{emoji} id ..."              / "    ✅ id (Xs)"
      iter_item:      "│  ├─ id ..."                / "│  └─ ✅ id (Xs)"
      sub_step(d=1):  "│  │  ├─ emoji id ..."       / "│  │  └─ ✅ id (Xs) → preview"

    所有输出经 _progress_print() 路由：
      - 持有 _progress_print_lock 避免并发 iteration 下 stdout 交错；
      - 打印前先擦掉底部心跳行，打印后让心跳线程下次刷新时自动重画，
        这样心跳行永远是"屏幕最后一行"、不会和节点日志互相覆盖产生残行。
    ...
    try:
        ...
        if status == "start":
            if is_iter_item:
                ...
                line = f"│  │  ├─ {emoji} {node_id}..."
            else:
                emoji = _NODE_EMOJI.get(node_type, "📦")
                line = f"{emoji} {node_id} ({node_type})..."
        ...
        # 预览（仅 sub_step / 顶层节点 ok 时有意义）
        preview = ""
        if status == "ok" and result and isinstance(result, dict):
            for k, v in result.items():
                ...
                if len(s) > 50:
                    s = s[:50] + "..."
                preview = f" → {k}: {s}"
                break  # 只展示第一个非 __ key

        if is_iter_item:
            # iteration_item 结束行：固定一行，无预览
            _progress_print(f"│  └─ {icon} {node_id} ({duration}s)")
        elif is_sub_step:
            _progress_print(f"│  │  └─ {icon} {node_id} ({duration}s){preview}")
        else:
            _progress_print(f"    {icon} {node_id} ({duration}s){preview}")
    except Exception as _e:
        # callback 出错绝不能向上抛（engine 的 future.result() 会把它当作 item 失败）
        try:
            _progress_print(f"[progress-callback-error] {_e}")
        except Exception:
            pass


def _progress_print(line: str) -> None:
    """统一出口：持锁 → 擦心跳 → 打日志 → 触发心跳重画。"""
    with _progress_print_lock:
        monitor = _HeartbeatMonitor.current()
        if monitor is not None:
            monitor._clear_line_unlocked()
        try:
            print(line, flush=True)
        finally:
            if monitor is not None:
                monitor._schedule_redraw_unlocked()
...
# ---------------------------------------------------------------------------
# Heartbeat Monitor — 真实活体信号驱动的底部状态行
# ---------------------------------------------------------------------------
...
class _HeartbeatMonitor:
    ...
    核心思想：
      信号来源不是本地计数器，而是 `workflow.agent_registry.snapshot()`。
      该 snapshot 读取每个正在运行的 AIAgent 的 `_last_activity_ts` /
      `_last_activity_desc`——这是 agent 在"发起请求 / 收到 token /
      执行工具"等关键点主动打的活体时间戳。只要 agent 真的卡住（HTTP
      挂起 / 死锁 / 无限循环），这个时间戳就会冻住，心跳行的 idle 秒数
      就会不断上涨，用户立刻能看出是"真卡"还是"正常慢"。

    显示策略：
      TTY:     底部占一行，\r\033[K 原地刷新，不与节点日志互相覆盖；
      非 TTY:  每 `_LOG_INTERVAL` 秒打一行普通日志，便于重定向到文件。

    并发打印：
      所有节点 print 经由 `_progress_print()` 路由；打印前 monitor 会
      擦掉底部心跳行，打印完下次 tick 会自动重画。心跳本身的写入和
      擦除也持有 `_progress_print_lock`，杜绝交错。
    """
    ...
    # 心跳行最多展示的 agent 数（其余折叠为 +N more）
    _MAX_SHOWN = 5

    # 全局单例，供 _progress_print 查询
    ...
    def _run_loop(self) -> None:
        interval = self._TTY_INTERVAL if self._is_tty else 0.5
    ...
    def _tick(self) -> None:
        ...
        with _progress_print_lock:
            if self._is_tty:
                self._render_tty_unlocked(snap)
            else:
                self._render_log_unlocked(snap)
    ...
    def _render_tty_unlocked(self, snap) -> None:
        ...
        # 截断到终端宽度，避免自动换行把心跳行变多行
        try:
            cols = os.get_terminal_size().columns
        except Exception:
            cols = 120
        # 减 1 留点余地，避免最后一列触发自动换行
        max_width = max(20, cols - 1)
        display = text
        # 粗略地按字符截断（emoji 会略微失真但不会破版）
        if len(display) > max_width:
            display = display[: max_width - 1] + "…"

        # \r 回行首 + ESC[2K 清整行，再写入，不换行
        sys.stdout.write("\r\033[2K" + display)
        sys.stdout.flush()
        self._line_active = True

    def _render_log_unlocked(self, snap) -> None:
        if not snap:
            return
        now = time.time()
    ...
    # ---- 供 _progress_print 使用 -------------------------------------------

    def _clear_line_unlocked(self) -> None:
        """擦掉当前心跳行（若有）。调用方必须已持有 _progress_print_lock。"""
        if not self._is_tty:
            return
    ...
    def _schedule_redraw_unlocked(self) -> None:
        """标记"下一个 tick 重画心跳"。实际重画发生在心跳线程下一轮。

        注意：不直接重画，因为调用方线程此刻可能还要继续打印更多内容。
        把重画交给心跳线程保证一致性。
        """
        # 什么都不用做：_line_active 已在 _clear_line_unlocked 置 False，
        # 下次 _render_tty_unlocked() 自然会重新 write。
        return